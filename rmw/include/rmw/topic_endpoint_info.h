...
typedef struct RMW_PUBLIC_TYPE rmw_topic_endpoint_info_s
{
  ...
  /// Serialized buffer backend metadata advertised for this endpoint
...
/// Set serialized buffer backend metadata in the given topic endpoint info data structure.
/**
 * Allocates memory and copies the serialized `buffer_backend_metadata` string
 * into the topic endpoint info.
 *
 * <hr>
 * Attribute          | Adherence
 * ------------------ | -------------
 ...
 *   given `topic_endpoint` while setting it.
 *   Access to C-style string arguments is read-only but it is not synchronized.
 *   Concurrent `buffer_backend_metadata` reads are safe, but concurrent reads and writes are not.
 *
 * \pre Given `buffer_backend_metadata` is a valid C-style string i.e. NULL terminated.
 *
 * \param[inout] topic_endpoint_info Data structure to be populated.
 * \param[in] buffer_backend_metadata Serialized buffer backend metadata to be set.
 * \param[in] allocator Allocator to be used.
 * \returns `RMW_RET_OK` if successful, or
 * \returns `RMW_RET_INVALID_ARGUMENT` if `topic_endpoint_info` is NULL, or
 * \returns `RMW_RET_INVALID_ARGUMENT` if `buffer_backend_metadata` is NULL, or
 * \returns `RMW_RET_BAD_ALLOC` if memory allocation fails, or
 * \returns `RMW_RET_ERROR` when an unspecified error occurs.
 * \remark This function sets the RMW error state on failure.
 */
